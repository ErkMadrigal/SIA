...
btnSearch.onclick = async () => {
    ...
    const payload = { action: "getEmpleadoBiometrico", query };
    ...
    try {
        ...
        // console.log("Foto del empleado cargada:", fotoEmpleadoBlob);
...
document.getElementById("btnLeer").addEventListener("click", async () => {
    ...
    if(res && res.score !== undefined){
        if(res.score.toFixed(4) >= 0.8999 && res.similar){
            ...
            Swal.fire({
                title: "Rostro no coincide. Acceso denegado.",
                icon: "error",
                confirmButtonColor: "#3085d6",
                confirmButtonText: "Cerrar",
                timer: 4000
            })
            resultado.innerHTML = `<b>Rostro no coincide. Acceso denegado.<b>`;
        }
    } else {
        resultado.innerHTML = "Error al comparar rostros.";
    }
});
